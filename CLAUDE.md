...
> - **Fix 500 `/analytics/finance/`** : `AnalyticsFinanceView` filtrait `TransactionCaisse.objects.filter(caisse__company=...)` — or `TransactionCaisse` n'a pas de FK `caisse` (mais `session`). Corrigé en `session__caisse__company=company` (FieldError → 500 réglé). Bug préexistant.
> - **Missions par type (FK client/fournisseur)** : `Mission.depot_depart`/`depot_arrivee` rendus **nullables** + ajout FK `client` (ventes.Client) & `fournisseur` (produits.Fournisseur) — migration `logistique/0007_mission_client_fournisseur`. `MissionCreateSerializer.validate()` impose par type : transfert→2 dépôts ; livraison→dépôt source+client ; enlevement→fournisseur+dépôt destination. List/Detail serializers exposent `type_mission`/`type_label`/`client(_nom)`/`fournisseur(_nom)`. Bon de livraison PDF rendu null-safe.
> - **Config caisse entreprise (admin)** : `PATCH /caisse-entreprise/configurer/` (action sur `CaisseEntrepriseViewSet`, `FINANCE_WRITE`=admin) → `get_or_create` (crée la caisse si absente : règle les entreprises créées avant l'auto-création) + applique `nom`/`devise`. Lecture inchangée (`/caisse-entreprise/me/`).
> - **Deep links Android (App Links)** : `config/deeplinks.py` sert `/.well-known/assetlinks.json` (package `com.example.mobile` + SHA-256 clé debug) + redirections de repli `/first-login` & `/reset-password` → `FRONTEND_URL` (si app non installée). Nouveau setting `DEEPLINK_BASE_URL` (défaut `https://gestion.kingreys.fr`). L'email first-login pointe désormais sur `DEEPLINK_BASE_URL/first-login?token=` → ouvre l'app si installée, sinon le web. ⚠️ MAJ du SHA-256 + package nécessaire au passage en clé de release.
